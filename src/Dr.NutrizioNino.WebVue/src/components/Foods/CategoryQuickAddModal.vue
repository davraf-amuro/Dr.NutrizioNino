...
import { createCategory } from '@/modules/categories/api/categories.api'
...
async function save() {
  formRef.value?.validate(async (errors) => {
    ...
    errorMessage.value = null
    try {
      const created = await createCategory({ name: formModel.name.trim() })
      emit('created', created)
      emit('update:show', false)
    } catch (e) {
      if (e instanceof ApiError && e.status === 409) {
        errorMessage.value = `La categoria "${formModel.name.trim()}" esiste già.`
      } else {
