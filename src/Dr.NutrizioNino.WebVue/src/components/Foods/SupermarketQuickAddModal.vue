<template>
  <n-modal
    ...
    preset="card"
    title="Aggiungi nuovo supermercato"
    style="width: 400px"
...
import { createSupermarket } from '@/modules/supermarkets/api/supermarkets.api'
...
const emit = defineEmits<{
  ...
  created: [supermarket: Supermarket]
}>()
...
async function save() {
  formRef.value?.validate(async (errors) => {
    ...
    errorMessage.value = null
    try {
      const created = await createSupermarket({ name: formModel.name.trim() })
      emit('created', created)
      emit('update:show', false)
    } catch (e) {
      if (e instanceof ApiError && e.status === 409) {
        errorMessage.value = `Il supermercato "${formModel.name.trim()}" esiste già.`
      } else {
