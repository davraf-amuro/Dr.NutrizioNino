<template>
  <n-space vertical size="medium">
    <n-h3 style="margin: 0">Nuova ricetta</n-h3>

    <n-spin :show="isLoading">
      <n-form :model="form" label-placement="left" label-width="120" label-align="right">
        <n-form-item label="Nome ricetta" path="name" :rule="{ required: true, min: 3, message: 'Nome obbligatorio (min 3 caratteri)', trigger: 'blur' }">
          <n-input v-model:value="form.name" :maxlength="50" placeholder="Inserisci il nome della ricetta" />
        </n-form-item>
      </n-form>
      ...
      <RecipeNutritionPreview :nutrients="nutrients" title="Valori totali (anteprima)" />
    </n-spin>
...
import { useRecipeCalculator, type RecipeIngredient } from '@/core/composables/useRecipeCalculator'
import RecipeIngredientList from './RecipeIngredientList.vue'
import RecipeNutritionPreview from './RecipeNutritionPreview.vue'
import type { CreateRecipeRequest } from '@/modules/recipes/api/recipes.api'
...
const { nutrients } = useRecipeCalculator(ingredients)
